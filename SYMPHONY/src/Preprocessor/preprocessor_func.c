...
{ 
   ...
   */ 
   ...
   int i, j, k, row_ind, elem_ind, *matind, *matbeg, *r_matind, *r_matbeg; 
...
/*===========================================================================*/
/*===========================================================================*/
int prep_find_lhs_params(rowpackedarray *row_P, MIPinfo mip_inf, int m, int n, 
			 double *ub, double *lb, lhsparams *lhs, 
			 int prep_level)
{
...
/* for now works only with bounded problems */
/* assumes all variables has bounds, so will not check */
/*===========================================================================*/
/*===========================================================================*/
int prep_find_row_advanced_bounds(MIPdesc *mip, lhsparams *lhs, 
				  prep_stats *stats, int max_iter, 
				  int keeptrack)
{
#if 0   
   int i, j, k, l, n, m, iter_cnt, row_ind; 
   int length, common_length, const_length, all_length; 
   double * ratios, *matval, *const_matval, *matind, *const_matind; 
   int *r_matbeg, *r_matind;
   int start, const_start, end, const_end, * solution, sol_size;   
   int sr_relax_num; 
   char bounds_considered;
   char *row_checked, row_checked_cnt;
   double *r_matval, min, max, *r_ubound, *r_lbound, temp_max, temp_min; 
   double * ub, *lb, *rhs; 
   char *sense; 
   double etol = stats->etol;
   int termcode = PREP_CONTINUE;
   
   double row_ratio = stats->single_row_relax_ratio;
   ...
   row_deleted = (char *) calloc(CSIZE, m);
   row_checked = (char *) malloc(CSIZE *m); 
   ratios = (double *) malloc(DSIZE * n);
   obj = (double *) malloc(DSIZE * n);
   common_inds = (int *) malloc(ISIZE *n);
   solution = (int *) malloc(ISIZE *n);
   temp_ind = (int *) malloc(ISIZE*n);

   /* FIXME! think about this - maybe try until we improve max/min?*/
   sr_relax_num = (int)(m * row_ratio); 
   sr_relax_num = sr_relax_num < m ? sr_relax_num : m;  
   iter_cnt = 0;
   
   while(termcode == PREP_CONTINUE && iter_cnt < max_iter){
      
      for(row_ind = 0; row_ind < m && row_ind++){
	     	 
	 
	 /* with the current bounds, see if we can preprocess anything*/
	 /* redundancy and infeasibility check */
	 termcode = check_basic_prep(r_lbound[row_ind], r_ubound[row_ind],
				     rhs[row_ind], sense[row_ind], 
				     
				     
		       
	 
	 memset(row_checked, FALSE, CSIZE*m);
	 row_checked_cnt = 0; /* when this is = m-1, stop! */
	 start = r_matbeg[row_ind];
	 end = r_matbeg[row_ind + 1];
	 
	 /* min/max of only considering bounds - obtained from collect mipinfo */
	 
	 max = temp_max = mip_inf->row_ub[row_ind];
	 min = temp_min = mip_inf->row_lb[row_ind];
	 
	 /* see if we can improve this considering one row relaxations*/
	 
	 length = end - start;	 
	 for(j = r_matbeg[row_ind]; j < r_matbeg[row_ind + 1]; j++){
	    /* consider the rows which has common elements */
	    /* i > row_ind since we will update bounds for both 
	       obj = row_ind & const = i and 
	       obj = i & const = row_ind 
	    */
	    if(row_checked_cnt >= m - 1 - row_ind){
	       break;
	    }
	    
	 for(i = matbeg[r_matind[j]]; i < matbeg[r_matind[j+1]]; i++){
	    if( i <= row_ind || row_checked[i]){
	       continue;
	    }
	    row_checked[i] = TRUE;
	    row_checked_cnt++;

	    /* now we will update the bounds for row_ind and i considering 
	      
	       max/min a_{row_ind}x
	         s.t   a_{i}x = b_i
		       l <= x <= u
	       ...
	       max/min a_{i}x
	          s.t  a_{row_ind}x = b_{row_ind}
		              l <= x <= u    
	    */ 
	       	    	      
	    /* check common components between rows 'row_ind' and 'i'*/
	 ...
	 obj_matind = &(r_matbeg[row_ind]);
	 obj_matval = &(r_matbeg[row_ind]);
	 
	 const_start = r_matbeg[i];
	 const_end = r_matbeg[i+1];
	 const_matval = &(r_matval[const_start]);
	 const_matind = &(r_matind[const_start]);
	 const_length = const_end - const_start;
	 tot_length = const_length < length ? length: const_length;	
	 /* fill in ratios of const_length
	 and take care of indices in obj_row\const_row */
	 /* have to go through tot_length */
      ...
      for( j = 0, k = 0; k < const_length; ){

	 if(const_matval[k] > 0){
	    temp_max -= const_matval[k] * ub[const_matind[k]];
	    temp_min -= const_matval[k] * lb[const_matind[k]];
	 }else {
	    temp_max -= const_matval[k] * lb[const_matind[k]];
	    temp_min -= const_matval[k] * ub[const_matind[k]];
	 }	
	 
	 if( j < length){
	    if(obj_matind[j] < const_matind[k]){
	       j++;
	    }else if (obj_matind[j] > const_matind[k]){
	       ratios[k] = 0.0;
	       obj[k] = 0.0;
	       k++;
	    } else{
	       common_inds[common_length] = obj_matind[j];
	       obj[k] = obj_matval[j];
	       ratios[k] = obj_matval[j]/const_matval[k];
	       common_length++;
	       k++;
	       j++;
	    }
      ...
      /* now, solve the lp relaxation of this problem */
      /* qsort the ratios */
      
      memcpy(temp_ind, const_matind, ISIZE*const_length);
      qsort_di(ratios, temp_ind, const_length);
      
      /* fill in the constraint till to hit the rhs*/
      /* report infeasible if cant fill */

      


      if(common_length <= 0){
	 if(bounds_considered){
	       /* nothing to be done, continue */
	    continue;
	 }else{
	    /* do ashus staff here */
	    bounds_considered = TRUE;
	 }
      }else{
	 /*benim fikir buraya */
