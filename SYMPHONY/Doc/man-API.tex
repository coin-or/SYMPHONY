...
This will allow the interactive shell to behave exactly like a Linux terminal
command line, i.e., it will keep the history of the used commands, will
complete the input word to possible commands, etc. Note that you must have the
required packages (readline and history) installed. 
...
of a \emph{main menu}, where an instance is read in and solved, a 
\emph{set menu}, where parameters are set, and a \emph{display menu}, where 
results, statistics and parameter values are displayed. 
 
\subsection{MS Windows}

To invoke SYMPHONY's interactive solver in an MS Windows environment, simply
double-click on the \code{symphony.exe} file in Windows Explorer. This should
open a terminal window in which the solver will run. Note that if you built
SYMPHONY in CYGWIN without the option \code{--enable-dos-compile}, then you
will have to have the CYGWIN DLL in your path in order for the executable to
run.
...
\begin{verbatim}
 *******************************************************
 *   This is SYMPHONY Version 5.1.0                    *
...
read in the data file (note that, in order to enable GMPL/AMPL reader, you
have to configure SYMPHONY with GLPK package - see Section \ref{building}):
{\color{Brown}
\begin{verbatim}
 SYMPHONY: load 
...
After loading the instance, you are free to type \code{solve} to solve the 
...
To enter this submenu, type \code{set}: 
{\color{Brown}
\begin{verbatim}
SYMPHONY: set
...
\code{verbosity} parameter (set to -1 by defualt for the interactive shell 
...
\code{help} or \code{?} displays only a subset of all 
...
name of that parameter. As an instance, assume that \code{my\_param} file  
...
process after a \code{solve} call. To enter this submenu and see available 
options, type \code{display} and then \code{help} or \code{?}:  
{\color{Brown}
\begin{verbatim}
 SYMPHONY: display
...
set paramters that cannot be set on the command line or to save parameter
...
the header file \texttt{symphony\_api.h}. Some of the basic commands are 
...
to develop custom applications is provided in Chapter~\ref{man-SYMPHONY}.

\section{Getting Help}
\label{resources}

The main point of entry for additiona help, trouble shooting, and
