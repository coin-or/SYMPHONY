...
To develop an applicationwith SYMPHONY, you need to first understand how the
...
configuration system, as well as a number of sudirectories, each of which
corresponds to a library used by SYMPHONY for some specific functionality. The
files assocaited with SYMPHONY itself are located in the \code{SYMPHONY}
...
for SYMPHONY itself. Within the \code{src} subdirectory, the files are
...
\code{Applications/} (see Sections~\ref{build_app_unix} and
\ref{build_app_msvc}) for instructions on building the applications). The
...
developing a custom application from scratch using SYMPHONY's callbacks. The
subdirectory \code{SYMPHONY/Applications/USER} contains the files needed for
implementing the callbacks and is essentially a template for developing an
application. In this directory and its subdirectories, which mirror the
...
module, initially called \code{USER/*/user\_*.c}, where \code{*} is the name
of the module. The primary thing that you, as the user, need to understand to
build a custom application is how to fill in these stubs. That is what the
second part of this capter is about. For information on building your
application once it is developed, you may want to skip ahead to
Section~\ref{building_custom_app}.
...
array itself and set \code{*array} to be the array allocated.  If an output
array is optional and the user is not returning any values in that array, then
the user {\em must not} set \code{*array} because this is how \BB\ decides
...
the function \code{get\_*\_ptr()} where * is the appropriate module (see the
...
are called \code{user\_free\_*}, where \code{*} is the module. These
...
then you simply need to set the parameter \code{*\_debug}, where * is
...
start \BB\ with the \code{-a} option. This will keep the cut
...
with the flag \code{--enable-cut-check} (see Section \ref{configuring}). 
Type \code{make clean} and then \code{make}.
