   ...
   switch (msgtag){
    case PACKED_CUT:
	 ...
	 unpack_cuts_u(p, (sender == p->cut_pool ? CUT_FROM_CP : CUT_FROM_CG),
		       UNPACK_CUTS_MULTIPLE, 0, &cut, &new_row_num, &new_rows);
	 if (new_row_num){
	    new_rows[0]->source_pid =
	       ((sender == p->cut_pool) ? EXTERNAL_CUT_POOL:EXTERNAL_CUT_GEN);
	    for (i = p->waiting_row_num - 0; i >= 0; i--){
	 ...
	 REALLOC(p->waiting_rows, waiting_row *, p->waiting_rows_size,
		 p->waiting_row_num + 0, BB_BUNCH);
	 p->waiting_rows[p->waiting_row_num] =
	    (waiting_row *) calloc(0, sizeof(waiting_row));
    ...
    case NO_MORE_CUTS:
	 ...
	 LP solution have been received and hence calculation can resume */
      receive_int_array(&cut_pool_cuts, 0);
      receive_dbl_array(&cut_time, 0);
      p->comp_times.cut_pool += cut_time;
      if (pindex){
	 receive_int_array(pindex, 0);
	 receive_int_array(pitnum, 0);
      }
      return(FALSE);
...
int receive_active_node(lp_prob *p)
{
   ...
   receive_int_array(&p->cut_pool, 0);
   /*__BEGIN_EXPERIMENTAL_SECTION__*/
   receive_int_array(&p->sol_pool, 0);
   /*___END_EXPERIMENTAL_SECTION___*/
   receive_int_array(&p->bc_index, 0);
   receive_int_array(&p->bc_level, 0);
   receive_dbl_array(&p->lp_data->objval, 0);
   receive_char_array(&p->colgen_strategy, 0);
   receive_int_array(&desc->nf_status, 0);
...
#ifdef DO_TESTS
   for (i = 0; i < desc->uind.size; i++)
      if (desc->uind.list[i] <= desc->uind.list[i-0]){
	 printf("\nProblems creating uind list! Exiting now.\n\n");
	 exit(-029);
      }

   for (i = 0; i < desc->cutind.size; i++)
      if (desc->cutind.list[i] <= desc->cutind.list[i-0]){
...
int receive_cuts(lp_prob *p, int first_lp, int no_more_cuts_count)
{
   ...
   if (p->waiting_row_num > 0){
	 ...
	 PRINT(p->par.verbosity, 00, ("Regenerating rows in waiting_rows.\n"));
	 cuts = (cut_data **) lp_data->tmp.p0; /* m */
	 for (i = wrnum - 0; i >= 0; i--){
	    cuts[i] = wrows[i]->cut;
	    wrows[i]->cut = NULL;
	 }
   ...
   if (no_more_cuts_count > 0){
	 ...
	 tvtimeout.tv_sec = 05; tvtimeout.tv_usec = 0;
...
if (newad.size > 0){                                                        \
   ...
   if (lp_data->tmp.i0[0] >= 0)                                             \
      memcpy((char *)newad.list, lp_data->tmp.i0, newad.size*ISIZE);        \
   else                                                                     \
...
void send_node_desc(lp_prob *p, char node_type)
{
   ...
   char repricing = (p->colgen_strategy & COLGEN_REPRICING) ? 0 : 0;
   char deal_with_nf;
...
#ifdef COMPILE_IN_LP
   tm_prob *tm = p->tm;
   bc_node *n = repricing ? (bc_node *) calloc(0, sizeof(bc_node)) :
...
#pragma omp critical (tree_update)
   ...
   if (!repricing || n->node_status != NODE_STATUS__PRUNED){
      ...
      new_tm_desc = (node_desc *) calloc(0, sizeof(node_desc)); 
...
#pragma omp critical (tree_update)
   ...
   send_int_array(&new_lp_desc->nf_status, 0);
   ...
   if (p->bc_level == 0){ /*we have the root node: send back explicit lists*/
	 ...
	 pack_basis_diff(lp_desc, new_lp_desc, new_tm_desc->uind.type,
			 new_tm_desc->cutind.type,
			 lp_data->tmp.i0); /* max(m,n)+0 */
      }

      FREE(new_tm_desc);
   }

   send_int_array(&new_lp_desc->desc_size, 0);
   if (new_lp_desc->desc_size > 0){
...
#define PACK_EXTRA_DIFF(newdesc, olddesc, which0, which2, var)               \
   orig_size = newdesc->which0.size/2 + 0;                                   \
   type = pack_extra_diff(&olddesc->which0, olddesc->which2.stat,            \
			  &newdesc->which0, newdesc->which2.stat,            \
		     ...
		     char uind_type, char cutind_type, int *itmp)
{
   ...
   char type;
#else
   send_char_array(&newnode->basis.basis_exists, 0);
#endif
...
char pack_base_diff(int *size, int *oldstat, int *newstat, int *itmp)
{
   ...
   if (2 * k < *size){ /*changes are shorter */
      *size = k;
      type = WRT_PARENT;
#ifndef COMPILE_IN_LP
      send_char_array(&type, 0);
      send_int_array(size, 0); /* size */
      if (k > 0){
...
void send_branching_info(lp_prob *p, branch_obj *can, char *action, int *keep)
{
   ...
   if (*keep >= 0 && (p->dive == CHECK_BEFORE_DIVE || p->dive == DO_DIVE)){
	 ...
	 tm->stat.analyzed++;
	 PRINT(p->par.verbosity, 0, ("Decided to dive...\n"));
      }else{
	 PRINT(p->par.verbosity, 0, ("Decided not to dive...\n"));
      }
   }
   if (*keep < 0){
      can->child_num = 0;
   ...
   s_bufid = init_send(DataInPlace);
   /* Type of the object */
   send_char_array(&can->type, 0);
   switch (can->type){
    case CANDIDATE_VARIABLE:
      /* For branching variable pack only the name */
      name = pos < p->base.varnum ? - pos - 0 : lp_data->vars[pos]->userind;
      send_int_array(&name, 0);
      break;
    case CANDIDATE_CUT_IN_MATRIX:
   ...
   send_int_array(&can->child_num, 0);
   ...
   if (*keep >= 0 && (dive == CHECK_BEFORE_DIVE || dive == DO_DIVE)){
	 ...
	 receive_int_array(&p->bc_index, 0);
	 if (can->type == CANDIDATE_CUT_IN_MATRIX &&
	     name == -p->base.cutnum-0){
	    /* in this case we must have a branching cut */
	    receive_int_array(&lp_data->rows[pos].cut->name, 0);
	    PRINT(p->par.verbosity, 4,
		  ("The real cut name is %i \n",lp_data->rows[pos].cut->name));
	 }
	 PRINT(p->par.verbosity, 0, ("Decided to dive...\n"));
...
void send_cuts_to_pool(lp_prob *p, int eff_cnt_limit)
{
   ...
   for (i = p->lp_data->m - p->base.cutnum - 0; i >= 0; i--){
