...
IP_INFEASIBLE} and return {\tt USER_SUCCESS}. If the user tests the
feasibility status of the solution and determines that the current solution is
feasible, then {\tt *feasible} should be set to {\tt IP_FEASIBLE} instead. IN
...
heur_solution}. In this case, {\tt *objval} should be set to the value of the
solution and {\tt *feasible} should be set to {\tt IP_HEUR_FEASIBLE}. 
...
IP\_INFEASIBLE}, {\tt IP_FEASIBLE}, or {\tt IP_HEUR_FEASIBLE}). \\
...
{\tt double *heur_solution} & OUT & Used to return a solution determined
heuristically.
\et
...
the description of \texttt{user\_find\_cuts()} in Section }{)}{user_find_cuts}
