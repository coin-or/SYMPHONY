%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{User-written functions of the LP module}
...
needed to describe a MIP. This data structure is used by SYMPHONY for two
purposes. First, it is used to store the description of a generic MIP that is
...
\hyperref{{\tt user\_send\_lp\_data()}} {{\tt user\_send\_lp\_data()} (see
...
Here, the user must receive here all problem-specific information sent from
...
user\_send\_lp\_data()}} {{\tt user\_send\_lp\_data()} (see Section
...
{\tt USER\_SUCCESS} & User received the data. \\
{\tt USER\_DEFAULT} & User function not implemented. \\
\et
...
\begin{verbatim}
int user_create_subproblem(void *user, MIPdesc *mip, int *indices, 
...
allocated in \texttt{user\_create\_subproblem()} are owned by SYMPHONY after
...
estimates that SYMPHONY can use to perform memory allocation. They do not
have to be exact numbers. In fact, these estimates need not be provided at
all. \\
\\
Note that, the user should return ``DEFAULT'' if an MPS or GLPMPL/AMPL file
was read in to describe the original MIP. \BB\ will allocate the corresponding
...
{\tt double *true_objval} & OUT & The user can return the ``true'' objective
...
valuesunless the return code is {\tt USER\_DEFAULT}.
\et
...
{\tt USER\_DEFAULT} & action is set to the value of the parameter {\tt
...
{\tt USER\_DEFAULT} & SYMPHONY prints out is own branching information. \\
\et
...
\hyperref{{\tt user\_find\_cuts()}} {{\tt user\_find\_cuts()} in Section
}{}{user_find_cuts}. These cuts can be constructed using the helper function
\hyperref{{\tt create\_explicit\_cut()}} {{\tt create\_explicit\_cut()} (see
Section }{)}{create_explicit_cut} and are packed and unpacked automatically,
so the user does not need to implement this function. In post processing,
SYMPHONY unpacks explicitly defined cuts and internally generated generic
cuts.
...
SYMPHONY deals with only exlicit cuts and internally generated cuts. 
\et
...
\texttt{*new\_cuts} to point to this array. Post-processing consists of
...
{\tt void *user} &  IN & Pointer to the user-defined LP data structure. \\
& & \\
{\tt LPdata *lp_data} & A pointer to SYMPHONY's internal data structure for
storing the LP relaxation and related data.
{\tt int varnum} & IN & The number of variables currently in the LP
relaxation. (The length of the {\tt *vars} and {\tt x} arrays.) \\
{\tt var\_desc **vars} & IN & The variables currently in the LP relaxation.\\
{\tt double *x} & IN & Values of the above variables.\\
{\tt int *new\_cut\_num} & OUT & The number of cuts generated. \\
{\tt cut\_data ***new\_cuts} & OUT & The cuts and the corresponding rows. \\
\et
...
{\tt GENERATE_CGL_CUTS} & Generate generic CGL cuts, according to parameter
settings. \\
{\tt DO_NOT_GENERATE_CGL_CUTS} & No additional cuts are generated. \\
\et
