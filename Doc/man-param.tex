...
\item[{\tt mc\_find\_nondominated\_solutions} -- boolean ({\tt TRUE}).]
If the problem to be solved is a bicriteria problem and if only the 
supported solutions are desired to be found, then the user should turn 
this parameter off before calling {\tt sym\_mc\_solve} routine.
...
mc\_find\_nondominated\_solutions & -boolean- \\
\hline \hline
