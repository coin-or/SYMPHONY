%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{User-written functions of the CP process}
...
\functiondef{user\_receive\_cp\_data}
...
(see Section }{)}{user_send_cp_data} function in the master process.
...
\item[Invoked from:] {\tt cp\_initialize} at process start.
...
\item[Invoked from:] cp\_close() at process shutdown.
...
\htmlref{user\_send\_lp\_solution()}{user_send_lp_solution}}
function of the LP process the user opted for packing the current LP
solution herself. Here she must receive the very same data she sent
there.
