...
\subsection{Getting a Quick Start}

Developing a simple branch and bound solver can be done quite quickly using
the sample framework provided in the \textt{SYMPHONY/User/} directory. The
...
can be developed by filling in only a few simple functions. Here are the
steps.
\begin{enumerate}
\item First, decide what data has to be stored to describe your instance. Edit
the file \texttt{User/include/user.h}, adding the appropriate fields to the 
\textt{user\_problem} structure to store a problem instance.
\item Now edit the file \texttt{User/Master/master_user.c}. If you choose to
...
can read in the instance data. Also, check out the function
\texttt{user\_set\_base()}.
\item Edit the file \texttt{User/LP/lp_user.c}. The function
...
\end{enumerate}
Now compile by setting the variable \texttt{USERROOT} in the make file to be
\texttt{\${ROOT)/User} and typing ``\texttt{make}'' (assuming that the make
