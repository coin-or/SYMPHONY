...
directory {\tt SYMPHONY-3.0.2/Template/}. There is one file for each module,
...
process. The primary make file resides in the {\tt SYMPHONY-3.0.2/} directory.
The user make file resides in the user's subdirectory, initially called {\tt
SYMPHONY-3.0.2/Template/}. There are a number of variables that must be set in
...
SYMPHONY-3.0.2/Template} by default). When you are ready, type ``{\tt make}''
to make the executables. \BB\ will create three new subdirectories---{\tt
SYMPHONY-3.0.2/Template/obj.*}, {\tt SYMPHONY-3.0.2/Template/bin.*}, and {\tt
SYMPHONY-3.0.2/Template/dep.*} where * is a the current architecture (defined
in the make file).
...
{\tt SYMPHONY-3.0.2/Vrp/Master/vrp.c}.
...
stub files in the {\tt SYMPHONY-3.0.2$\backslash$Template} hierarchy are
filled in, use the project ``{\tt vrp}'' as a template to create a new project
called ``{\tt user}'' that compiles the files in {\tt
SYMPHONY-3.0.2$\backslash$Template} and creates a library. Modify the project
