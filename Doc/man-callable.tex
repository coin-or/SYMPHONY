...
array is requested, such as the aray of lower bounds on the variables, for
...
argument to all other API subroutines. This routne also invokes the callback
...
this is the case when the user specifies the location of an MPS or GMPL file
using the \texttt{-F} switch or when the user specifies the location of a
parameter file with the \texttt{-f} switch. This command also invokes the user
callback function \hyperref{{\tt user\_readparams()}} {{\tt
user\_readparams()} (see Section }{)}{user_readparams}.
...
\bt{lp{317pt}}
{\tt ERROR\_\_USER} & Error. User error detected in one of {\tt
user\_send\_lp\_data()}, {\tt user\_send\_cg\_data()}, {\tt
user\_send\_cp\_data()}, {\tt user\_receive\_feasible\_solution()}, 
...
{\tt } 
{\tt ERROR\_\_USER} & Error. User error detected in one of {\tt
user\_send\_lp\_data()}, {\tt user\_send\_cg\_data()}, 
...
\firstfuncdef{sym\_get\_num\_cols}
\begin{verbatim}
