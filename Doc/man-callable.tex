...
argument to all other API subroutines. 
...
\functiondef{sym\_set\_defaults}
\begin{verbatim}
...
This routine reads in the command line arguments. It has to be called when 
problem description is to be given in MPS, GMPL/AMPL or user defined format 
or a parameter (parameter file) is desired to be set (read in) 
by the command line.  
...
This routine is used to set the user problem structure of the environment.
...
format or by the {\tt user\_io} callback. If the problem is to be loaded from 
an MPS or a GMPL/AMPL file, then the {\tt sym\_parse\_command\_line} function 
...
\bt{lp{317pt}}
{\tt ERROR\_\_USER} & Error. User error detected in one of {\tt user\_io}, 
...
This routine attempts to find lower and upper bounds for the problem through
the user defined heuristics in {\tt start\_heurs\_u} user callback. 
...
This routine solves the corresponding MILP problem. 
It may also be used to continue solving a problem stopped for modification 
of some parameters.In this case, SYMPHONY should know that the warm start 
structure constructed during the solution of the original problem 
is going to be used. Thus, ``warm\_start'' parameter should have been set to 
true before solving from where it was left off. For instance:

\begin{verbatim}
...
sym_set_int_param(env, ``keep_description_of_pruned'', TRUE); 
sym_set_int_param(env, ``find_first_feasible'', TRUE); 
sym_set_int_param(env, ``node_selection_strategy'', DEPTH_FIRST_SEARCH); 
sym_initial_solve(env); 
sym_set_int_param(env, ``find_first_feasible'', FALSE); 
sym_set_int_param(env, ``node_selection_strategy'', BEST_FIRST_SEARCH); 
sym_set_int_param(env, ``warm_start'', TRUE);
sym_solve(env); 
... 
\end{verbatim}

However,  {\tt sym\_resolve} routine should be called if the modification is
on the problem data. 

Note that, the environment parameter ``keep\_description\_of\_pruned'' should 
be set to true if it is desired to keep the warm start information for further 
usage: warm starting, resolving or basic sensitivity analysis.
...
\bt{lp{317pt}}
{\tt ERROR\_\_USER} & Error. User error detected in one of {\tt user\_send\_
lp\_data}, {\tt user\_send\_cg\_data}, {\tt user\_send\_cp\_data}, 
{\tt user\_receive\_feasible\_solution}, {\tt user\_display\_solution}, 
...
\functiondef{sym\_initial\_solve}
\begin{verbatim}
...
This routine solves the corresponding MILP problem from scratch even in the 
presence of a loaded warm start. Nonetheless, SYMPHONY will differantiate 
the problem data modification and will solve the modified problem (instead of 
...
\bt{lp{317pt}}
{\tt ERROR\_\_USER} & Error. User error detected in one of {\tt user\_send\_lp\_data}, {\tt user\_send\_cg\_data}, {\tt user\_send\_cp\_data}, 
{\tt user\_receive\_feasible\_solution}, {\tt user\_display\_solution}, 
{\tt user\_process\_own\_messages} functions. \\ 
{\tt TM\_OPTIMAL\_SOLUTION\_FOUND} & Tree Manager (TM) found the optimal solution and stopped.\\ 
{\tt TM\_TIME\_LIMIT\_EXCEEDED} & TM stopped after reaching the predefined 
...
This routine resolves the corresponding modified problem after a warm start 
has been loaded or necessary parameter or problem data modification is done 
on the existing warm start structure.  Note that, unless an explicit warm 
start has been loaded, in order to use this functionality,
``keep\_description\_of\_pruned''parameter should have been set to true before 
solving the original problem. This will ask SYMPHONY to keep the necessary 
warm starting and resolving information. 

%In addition to that, if the resolve process is desired to be 
%incurred on the warm start structure loaded to the environment
%during the solution of the original problem, SYMPHONY should know that
%the warm start structure constructed from the original problem and loaded 
%by default is going to be used. Thus, ``warm_start'' parameter should have
%been set to on before resolving. For instance:
%... \\
%sym_set_int_param(env, ``keep_description_of_pruned'', TRUE); \\
%sym_set_int_param(env, ``find_first_feasible'', TRUE); \\
%sym_set_int_param(env, ``node_selection_strategy'', DEPTH_FIRST_SEARCH); \\
...
The second objective function's coefficients can be set by calling
{\tt sym\_set\_obj2\_coeff} function. 
...
This routine is used to save the global cuts to the cut pool of 
the environment.    
...
\bt{lp{317pt}}
{\tt INT} & The number of the cut pools stored in. 
\et
\ed
\vspace{1ex}
...
This routine is used to load the user defined problem description to SYMPHONY
...
{\tt char make\_copy} & IN & SYMPHONY will create the copies of the
...
{user\_initialize\_root\_node} function. \\
{\tt FUNCTION\_TERMINATED\_ABNORMALLY} & Function invoked unsuccessfully.\\
{\tt FUNCTION\_TERMINATED\_NORMALLY} & Function invoked successfully. \\
\et
\ed
...
\functiondef{sym\_is\_abandoned}
\begin{verbatim}
...
This query routine is used to learn about the problem status after 
solving it: Is problem abandoned for some reason?
...
\bt{lp{317pt}}
{\tt TRUE} & The problem is abandoned. \\
{\tt FALSE} & The problem is not abandoned.\\
{\tt FUNCTION\_TERMINATED\_ABNORMALLY} & Function invoked unsuccessfully.\\
\et
\ed
\vspace{1ex}
...
This query routine is used to learn about the problem status after 
solving it: Is problem solved to optimality?
...
\bt{lp{317pt}}
{\tt TRUE} & The problem is solved to optimality. \\
{\tt FALSE} & The problem is not solved to optimality.\\
{\tt FUNCTION\_TERMINATED\_ABNORMALLY} & Function invoked unsuccessfully.\\
\et
\ed
\vspace{1ex}
...
This query routine is used to learn about the problem status after 
solving it: Is problem proven to be infeasible?
...
\bt{lp{317pt}}
{\tt TRUE} & The problem is proven to be infeasible. \\
{\tt FALSE} & The problem is not proven to be infeasible.\\
{\tt FUNCTION\_TERMINATED\_ABNORMALLY} & Function invoked unsuccessfully.\\
\et
\ed
\vspace{1ex}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% sym_is_primal_objective_limit_reached
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\functiondef{sym\_is\_primal\_objective\_limit\_reached}
\begin{verbatim}
...
This query routine is used to learn about the problem status after 
solving it: Is the objective limit ( or target gap) achieved?
...
\bt{lp{317pt}}
{\tt TRUE} & The goal is achieved. \\
{\tt FALSE} & The goal is not achieved.\\
{\tt FUNCTION\_TERMINATED\_ABNORMALLY} & Function invoked unsuccessfully.\\
\et
\ed
\vspace{1ex}
...
This query routine is used to learn about the problem status after 
solving it: Is iteration limit (or node limit) reached? It can also be used
to query the solution status if ``find\_first\_feasible'' parameter 
is set to true.
...
This query routine is used to learn about the problem status after 
solving it: Is time limit reached?
...
\bt{lp{317pt}}
{\tt TRUE} & Time limit is reached. \\
{\tt FALSE} & Time limit is not reached.\\
{\tt FUNCTION\_TERMINATED\_ABNORMALLY} & Function invoked unsuccessfully.\\
\et
\ed
\vspace{1ex}
...
This query routine is used to learn about the problem status after 
solving it: Is target gap achieved?
...
\bt{lp{317pt}}
{\tt TRUE} & Target gap is reached. \\
{\tt FALSE} & Target gap is not reached.\\
{\tt FUNCTION\_TERMINATED\_ABNORMALLY} & Function invoked unsuccessfully.\\
\et
...
\bt{lp{317pt}}
{\tt INT } & A positive integer indicating the number of the columns. \\
{\tt FUNCTION\_TERMINATED\_ABNORMALLY} & Function invoked unsuccessfully.\\
\et
\ed
\vspace{1ex}
...
\bt{lp{317pt}}
{\tt INT} & A positive integer indicating the number of the rows.\\
{\tt FUNCTION\_TERMINATED\_ABNORMALLY} & Function invoked unsuccessfully.\\
\et
\ed
\vspace{1ex}
...
This routine is used to get the number of the non zero entries in the 
...
\bt{lp{317pt}}
{\tt INT }& A positive integer indicating the number of the non zero 
elements. \\
{\tt FUNCTION\_TERMINATED\_ABNORMALLY} & Function invoked unsuccessfully.\\
\et
\ed
...
\bt{lp{317pt}}
{\tt NULL} & There is no problem description loaded to the environment. \\
{\tt DOUBLE *} & Array of the lower bounds of the variables. \\ 
\et
\ed
\vspace{1ex}
...
\bt{lp{317pt}}
{\tt NULL }& There is no problem description loaded to the environment. \\
{\tt DOUBLE }* & Array of the upper bounds of the variables. \\ 
\et
\ed
\vspace{1ex}
...
\bt{lp{317pt}}
{\tt NULL }& There is no problem description loaded to the environment. \\
{\tt CHAR *} & Array of the row senses. \\ 
\et
\ed
\vspace{1ex}
...
\bt{lp{317pt}}
{\tt NULL }& There is no problem description loaded to the environment. \\
{\tt DOUBLE * }& Array of the right hand side values. \\ 
\et
\ed
\vspace{1ex}
...
\bt{lp{317pt}}
{\tt NULL }& There is no problem description loaded to the environment. \\
{\tt DOUBLE * } & Array of the row ranges. \\ 
\et
\ed
\vspace{1ex}
...
\bt{lp{317pt}}
{\tt NULL }& There is no problem description loaded to the environment. \\
{\tt DOUBLE *} & Array of the lower bounds of the rows. \\ 
\et
\ed
\vspace{1ex}
...
\bt{lp{317pt}}
{\tt NULL }& There is no problem description loaded to the environment. \\
{\tt DOUBLE *} & Array of the upper bounds of the rows. \\ 
\et
\ed
\vspace{1ex}
...
This routine is used to get the objective coefficients.
...
\bt{lp{317pt}}
{\tt NULL }& There is no problem description loaded to the environment. \\
{\tt DOUBLE *} & Array of the objective coefficients. \\ 
\et
\ed
\vspace{1ex}
...
\bt{lp{317pt}}
{\tt 1 }& It is a minimization problem. \\
{\tt -1 } & It is a maximization problem. \\
{\tt FUNCTION\_TERMINATED\_ABNORMALLY} & Function invoked unsuccessfully.\\
\et
\ed
\vspace{1ex}
...
\bt{llp{250pt}}
{\tt sym\_environment *env} & IN & Pointer to the SYMPHONY environment.\\
{\tt int index} & IN & The index of the queried variable.
\et

\returns

\bt{lp{317pt}}
{\tt TRUE }& The variable is continuos. \\
{\tt FALSE }& The variable is not continuos. \\
{\tt FUNCTION\_TERMINATED\_ABNORMALLY} & Function invoked unsuccessfully.\\
\et
\ed
\vspace{1ex}
...
\bt{lp{317pt}}
{\tt TRUE }& The variable is binary. \\
{\tt FALSE }& The variable is not binary. \\
{\tt FUNCTION\_TERMINATED\_ABNORMALLY} & Function invoked unsuccessfully.\\
\et  
\ed
\vspace{1ex}
...
\bt{llp{250pt}}
{\tt sym\_environment *env} & IN & Pointer to the SYMPHONY environment.\\
{\tt int index} & IN & Index of the queried variable.
\et

\returns

\bt{lp{317pt}}
{\tt TRUE }& The variable is integer. \\
{\tt FALSE }& The variable is not integer. \\
{\tt FUNCTION\_TERMINATED\_ABNORMALLY} & Function invoked unsuccessfully.\\
\et  
\ed
\vspace{1ex}
...
This routine is used to get the problem solution.
...
\bt{lp{317pt}}
{\tt NULL }& There is no solution found. \\
{\tt DOUBLE *} & An array of the column solutions. \\ 
\et  
\ed
\vspace{1ex}
...
\bt{lp{317pt}}
{\tt NULL }&  Function invoked unsuccessfully or there is no solution found. \\
{\tt DOUBLE *} & Array of the row activities. \\ 
\et  
\ed
\vspace{1ex}
...
\bt{lp{317pt}}
{\tt INFINITY }& There is no solution found.\\
{\tt DOUBLE * }& A double indicating the objective value. \\ 
\et  
\ed
\vspace{1ex}
...
This routine is used to get the priori upper bound for the problem via the 
user defined heuristics in {\tt user\_start\_heurs } user callback function. 
Note that {\tt sym\_find\_initial\_bounds} function should be invoked before to
activate those heuristics. 
...
\bt{lp{317pt}}
{\tt INFINITY }& There is no priori upper bound.\\
{\tt DOUBLE } & A double indicating the primal bound. \\
\et  
\ed
\vspace{1ex}
...
\begin{verbatim}
double *sym_get_iteration\_count(sym_environment *env)
\end{verbatim}
...
\bt{lp{317pt}}
{\tt INT *} & An integer indicating the number of the anayzed nodes. \\ 
{\tt FUNCTION\_TERMINATED\_ABNORMALLY} & Function invoked unsuccessfully.\\
...
\functiondef{sym\_set\_obj\_coeff}
\begin{verbatim}
...
{\tt index} is out of the range of [0, number of variables-1] \\
\et  
\ed
\vspace{1ex}
...
\bt{llp{250pt}}
{\tt sym\_environment *env} & INOUT & Pointer to the SYMPHONY environment. \\
{\tt int index} & IN & Index of the variable. \\
...
\bt{llp{250pt}}
{\tt sym\_environment *env} & INOUT & Pointer to the SYMPHONY environment. \\
{\tt int index} & IN & Index of the row. \\
...
{\tt FUNCTION\_TERMINATED\_NORMALLY} & Function invoked successfully.\\
{\tt FUNCTION\_TERMINATED\_ABNORMALLY} & Function invoked unsuccessfully or
{\tt index} is out of the range of [0, number of rows-1] \\
\et  
\ed
\vspace{1ex}
...
This routine is used to set the objective sense.
...
\bt{llp{250pt}}
{\tt sym\_environment *env} & INOUT & Pointer to the SYMPHONY environment. \\
{\tt double *colsol} & IN &  An array of the known column solutions.
\et
...
{\tt FUNCTION\_TERMINATED\_NORMALLY} & Function invoked successfully.\\
{\tt FUNCTION\_TERMINATED\_ABNORMALLY} & Function invoked unsuccessfully or the 
given solution is not feasible. \\
\et  
\ed
\vspace{1ex}
...
This routine is used to set a priori upper bound to the problem.

\args

\bt{llp{250pt}}
{\tt sym\_environment *env} & INOUT & Pointer to the SYMPHONY environment. \\
{\tt double double} & IN &  The value of the priori upper bound.
\et
...
\bt{llp{250pt}}
{\tt sym\_environment *env} & INOUT & Pointer to the SYMPHONY environment. \\
{\tt int index} & IN &  The index of the variable to be modified. 
\et
...
\bt{llp{250pt}}
{\tt sym\_environment *env} & INOUT & Pointer to the SYMPHONY environment. \\
{\tt char **colname} & IN &  Double array including the column names. \\ 
\et
...
\bt{llp{250pt}}
{\tt sym\_environment *env} & INOUT & Pointer to the SYMPHONY environment. \\
{\tt int num\_elements} & IN & An integer indicating the non zero elements of
the column. \\
{\tt int *indices} & IN & An array indicating the row indices of the non zero
elements of the column. \\
{\tt double *elements} & IN & An array indicating the values of the non zero
elements of the column. \\
...
of the column. \\
{\tt char *name} An array including the name of the column.  
\et
...
{\tt int num\_elements} & IN & An integer indicating the non zero elements of
the row. \\
{\tt int *indices} & IN & An array indicating the column indices of the non zero
elements of the row. \\
{\tt double *elements} & IN & An array indicating the values of the non zero
elements of the row. \\
...
\bt{llp{250pt}}
{\tt sym\_environment *env} & INOUT & Pointer to the SYMPHONY environment. \\
{\tt int num} & IN & An integer indicating the number of columns to be deleted.\\
{\tt int *indices} & IN & An array indicating the indices of the columns to 
be deleted.
\et
...
\bt{lp{317pt}}
{\tt FUNCTION\_TERMINATED\_NORMALLY} & Function invoked successfully.\\
{\tt FUNCTION\_TERMINATED\_ABNORMALLY} & Function invoked unsuccessfully or one 
of the {\tt indices} is out of the range of [0, number of variables-1] \\
\et  
\ed
\vspace{1ex}
...
{\tt FUNCTION\_TERMINATED\_NORMALLY} & Function invoked successfully.\\
{\tt FUNCTION\_TERMINATED\_ABNORMALLY} & Function invoked unsuccessfully or
one of the {\tt indices} is out of the range of [0, number of variables-1] \\
\et  
\ed
\vspace{1ex}
...
\functiondef{sym\_write\_warm\_start\_desc}
\begin{verbatim}
...
\bt{lp{317pt}}
{\tt NULL} & SYMPHONY can not open the warm start file to read. \\
{\tt WARM\_START\_DESC *} & Pointer to a warm start structure read in from the given file.\\
\et  
\ed
\vspace{1ex}
...
environment. Note that, in order to use this functionality, 
``keep\_description\_of\_pruned'' parameter should have been set to true 
before solving the original problem. This will ask SYMPHONY to keep the 
necessary warm start and resolve information.

\args

\bt{llp{250pt}}
{\tt sym\_environment *env} & IN & Pointer to the SYMPHONY environment. \\
{\tt int copy\_warm\_start} & IN & A binary indicating whether the warm start
of the environment is desired to be copied ( 1 ) or overtaken ( 0 ). 
\et

\returns

\bt{lp{317pt}}
{\tt NULL} & There is no loaded warm start structure.\\
{\tt WARM\_START\_DESC * }& Pointer to a warm start structure copied or 
overtaken from the environment.\\
\et  
\ed
\vspace{1ex}
...
{\tt int value} & New value of the corresponding parameter.
\et
\ed
\vspace{1ex}
...
{\tt sym\_environment *env} & INOUT & Pointer to the SYMPHONY environment. \\
{\tt char *key} & IN & The name of the parameter to be set. \\
{\tt double value} & New value of the corresponding parameter.
\et
\ed
\vspace{1ex}
...
{\tt sym\_environment *env} & INOUT & Pointer to the SYMPHONY environment. \\
{\tt char *key} & IN & The name of the parameter to be set. \\
{\tt char *value} & New value of the corresponding parameter.
\et
\ed
\vspace{1ex}
...
\functiondef{sym\_create\_copy\_mip\_desc}
\begin{verbatim}
...
\bt{lp{317pt}}
{tt NULL} & An empty environment is passed in or there is no problem
...
\begin{verbatim}
sym\_environment *sym_create_copy_environment(sym_environment *env)
\end{verbatim}
...
\bt{lp{317pt}}
{tt NULL} & An empty environment is passed in. \\
...
\functiondef{sym\_get\_lb\_for\_new\_rhs}
\begin{verbatim}
...
the original solved problem. Note that, in order to use this feature, 
the ``keep\_description\_of\_pruned'' and ``sensitivity\_analysis'' parameters
need to be set to true before solving the original problem. 
\args
...
\bt{lp{317pt}}
{\tt FUNCTION\_TERMINATED\_ABNORMALLY} & Function invoked unsuccessfully, 
trying to read an empty problem description, tree nodes were not kept in 
memory or the sensitivity analysis flag was not set on before solving the 
problem to keep the needed information.\\
{\tt DOUBLE } & A double indicating the lower bound of the modified problem.\\
\et  
\ed
\vspace{1ex}
...
The OSI SYMPHONY methods are implemented simply as
wrapped calls to the SYMPHONY callable library. For instance, when an instance 
of the OSI interface class is constructed, a call is made to
...
branchAndBound & sym\_solve & 
solve the MILP problem. \\ \hline \hline
resolve & sym\_resolve & 
resolve the MILP problem after some modifications.\\ \hline \hline
initialSolve & sym\_initial\_solve & 
...
load the probkem through a set of arrays. \\ \hline \hline
...
find a lower bound to the new problem.\\ \hline \hline
...
query the problem status. \\ \hline \hline
isPrimalObjectiveLimitReached & sym\_is\_primal\_objective\_limit\_reached & 
query the problem status. \\ \hline \hline
...
isContinuous & sym\_is\_continuos & 
...
getInfinity & - & 
get the infinity definiton of SYMPHONY. \\ \hline \hline
