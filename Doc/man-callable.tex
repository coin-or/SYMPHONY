...
This routine attempts to find bounds for the problem through
the user defined heuristics if loaded to {\tt start\_heurs\_u} user callback. 
...
presence of a loaded warm start. Nonetheless, if any parameter or problem data
has been changed, SYMPHONY will know about those modifications and hence, will
solve the modified problem instead of the original one. However note that, 
any warm start information loaded or kept before will be deleted from the 
environment!
...
have been changed or problem data has been modified on the loaded warm start.
Note that, the user can keep a warm start by setting the ``keep\_warm\_start''
evironment parameter before calling a solve routine. So that, SYMPHONY 
will collect the necessary warm starting information through the 
solve procedure.
...
The second objective function's coefficients can always be set or altered 
by calling {\tt sym\_set\_obj2\_coeff} function.
...
This post-solution query routine is used to learn whether the objective
limit (target gap) was achieved.
...
\bt{lp{317pt}}
{\tt TRUE} & The goal was achieved. \\
{\tt FALSE} & The goal was not achieved.\\
\et
\ed
\vspace{1ex}
...
parameter wass set to true before solving the problem.
...
constraint matrix of the loaded problem.
...
This routine is used to get the second objective vector if loaded before.
...
This routine is used to get the priori upper/lower bound for the problem via 
the user defined heuristics if loaded to {\tt user\_start\_heurs } user 
callback function. Note that, {\tt sym\_find\_initial\_bounds} function should
be invoked before to activate those heuristics. 
...
of the column. \\
{\tt char *name} Pointer to a string of the name of the column.  
\et
...
``sensitivity\_analysis'' parameter needs to be set before solving 
