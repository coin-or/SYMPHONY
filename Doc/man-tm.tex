...
\subsection{User-written functions of the Master module}
...
{\tt USER\_DEFAULT} & Use function not implemented. \\
\et
...
\texttt{filename}, specified on the command line using the ``-f'' switch. The 
...
in the file \texttt{SYMPHONY-4.0/USER/user\_master.c}. Also, see the file
...
\texttt{SYMPHONY-4.0/USER/user\_master.c}. Upper case letters are reserved for
...
`\texttt{USER\_DEFAULT}, then \BB\ will look for the command-line switch
\texttt{-F} to specify the file name for reading in the model from either a
MPS or a GMPL/AMPL file. The \texttt{-D} command-line switch is used to
specify an additional data file for GMPL/AMPL models. If the \texttt{-D}
...
{\tt USER\_SUCCESS} & User parameters were read successfully. \\
{\tt USER\_DEFAULT} & SYMPHONY will read in the problem instance from either
an MPS or an GMPL/AMPL file. The command-line switches \texttt{-F} and
\texttt{-D} will be used to specify the model file and (in the case of
GMPL/AMPL) the data file.  \\
\et
...
{\tt USER\_DEFAULT} & Function not implemented. \\
\et
...
{\tt USER\_DEFAULT} & User function not implemented (someday, there may be a
...
\ref{variables} for a discussio of base and extra variables). If the function
...
putposes. If the \texttt{colnames} array is allocated, then SYMPHONY will use
...
solutions, as well as display them, will suffice. These routines simply
display all nonzeros by either index or name, depending on whether the user
set the column names. See
\hyperref{{\tt user_send_feasible_solution()}} {{\tt
user\_receive\_lp\_data()} in Section }{}{user_send_feasible_solution} for
more discussion.
...
\hyperref{{\tt user_receive_lp_data}} {{\tt user\_receive\_lp\_data()} in
...
See \hyperref{{\tt user_receive_cg_data}} {{\tt user\_receive\_cg\_data()} in
...
{\tt COMPILE\_IN\_LP} or {\tt COMPILE\_IN\_TM} are {\tt FALSE} in the {\tt
make} file), then the data will be sent and received through message-passing.
See \hyperref{{\tt user_receive_cp_data}} {{\tt user\_receive\_cp\_data()} in
